...
double MP3Track::get_quality_score() const {
    ...
    double final_score = ( base_score < 0) ? 0.0 : ((base_score > 100) ? 100 : base_score);
    return final_score; 
}
...
PointerWrapper<AudioTrack> MP3Track::clone() const {
    ...
    MP3Track *cloned = new MP3Track(*this);
    return PointerWrapper<AudioTrack>(cloned); // Replace with your implementation
}